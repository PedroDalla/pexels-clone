...
export const Image: React.FC<ImageProps> = ({ imageInfo, setCurrentImage }) => {
  return (
    <StyledImage
      onClick={() => {
      ...
      <div id="photo-controls">
        <div id="controls-header" className="control-buttons">
          <button>
            <FiBookmark size="22px"></FiBookmark>
          </button>
          <button>
            <FiHeart size="22px"></FiHeart>
          </button>
        </div>
        <div id="controls-footer">
          {/* <a id="author-link">
            {imageInfo.photographer_photo ? (
              <img src={imageInfo.photographer_photo} alt="Author"></img>
            ) : (
